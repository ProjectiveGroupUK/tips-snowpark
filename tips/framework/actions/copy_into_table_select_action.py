...
from tips.framework.metadata import table_metadata


class CopyIntoTableSelectAction(SqlAction):
    _source: str
    _target: str
    _binds: List[str]
    _fileFormatName: str
    ...
    def getCommands(self) -> List[object]:
        ...
        #using target table to infer number of required fields
                #retrieve columns of target table
        columns = table_metadata.getColumns(tableName=self._target, excludeVirtualColumns=False)
        select_clause = ", ".join([f"${i}" for i in range(1, len(columns) + 1)])

        
        # # Render the template with the necessary parameters
        # cmd: str = SQLTemplate().getTemplate(
        #     sqlAction="copy_into_file_select",
        #     parameters={
        #         "tableName": self._target,
        #         "selectClause": select_clause,
        #         "stageName": self._source,  # Assuming _source is the stage name
        #     },
        # )
        
        # # Wrap the command in an SQLCommand object and return it in a list
        # return [SQLCommand(sqlCommand=cmd)]

        cmd: str = SQLTemplate().getTemplate(
            sqlAction="copy_into_table_select",
            parameters={
                "fileName": sourceName,
                "tableName": self._target,
